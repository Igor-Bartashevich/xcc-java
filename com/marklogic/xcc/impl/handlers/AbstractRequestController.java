    ...
    public ResultSequence runRequest(ConnectionProvider provider, Request request, Logger logger)
        ...
        for (; t < tries; t++) {
            ...
            if (session.getTxnID() == null && 
               session.getTransactionMode() != Session.TransactionMode.AUTO) {
                if(re != null && 
...
//		setSendHeader ("Host", connInfo.getHost() + ":" + connInfo.getPort());
        ...
        if(session.getSessionID() != null) {
            ...
            logger.fine("Sending SessionID: " + session.getSessionID() + ", TxnID: " + session.getTxnID() + ", TxnMode: " + session.getTransactionMode());
        }
    }
}
